...
(function () {
    var BSTable = function (bstableId, url, columns, height) {
        ...
        this.height = height == null ? 665 : height;						//默认表格高度665
        this.data = {};
        this.queryParams = {}; // 向后台传递的自定义参数
        this.onClickCell = function (field, value, row, $element) {
            //自定义函数 Cell点击触发
        };
        this.showFooter = false;
        this.responseSuccessFun = function (data) {
            //自定义函数 Response Success 触发
            return data;
        };
        this.responseErrorFun = function (status, jqXHR) {
            //自定义函数 Response Error 触发
        };
        //没有匹配的结果显示的文字
        this.formatNoMatches_displaywords = "没有找到匹配的记录";
        ...
        init: function () {
            ...
            this.btInstance =
                $('#' + tableId).bootstrapTable({
                    ...
                    height: this.height,
                    showFooter: this.showFooter, //设置footer显示
                    icons: {
                    ...
                    , onClickCell: function (field, value, row, $element) {
                        me.onClickCell(field, value, row, $element);
                    }
                    , responseHandler: function (data) {
                        return me.responseSuccessFun(data);
                    }
                    , onLoadError: function (status, jqXHR) {
                        me.responseErrorFun(status, jqXHR);
                    }
                    , formatNoMatches: function () {
                        //没有匹配的结果
                        return me.formatNoMatches_displaywords;
                    }
